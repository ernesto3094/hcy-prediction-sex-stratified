  ...
  for (group_name in names(stable_groups)) {
    ...
    for (degree_value in polynomial_degrees) {
      ...
      if (is.null(fitted_model)) {
        ...
        # caret::train with method = "lm" returns one row of CV results.
...
# ------------------------------------------------------------
# Logic:
#   1. Find the maximum Mean_CV_R2 within each stability group.
#   2. Keep all degrees within 0.01 of that maximum.
#   3. Select the smallest degree among those.
#
# This avoids selecting a very high degree when the improvement is negligible.
...
# ------------------------------------------------------------
# This is useful if we want to use one degree for all groups.
# It selects the most frequently selected degree across groups.
# If there is a tie, the smaller degree is selected.
