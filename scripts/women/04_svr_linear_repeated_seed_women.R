...
# ------------------------------------------------------------
# 10. Train and evaluate linear SVR
# ------------------------------------------------------------

train_and_evaluate_svr_linear <- function(
    group_name,
    predictors,
    train_data,
    test_data,
  ...
  best_c <- fitted_model$bestTune$C
...
# ------------------------------------------------------------
# 11. Run repeated-seed linear SVR experiment
# ------------------------------------------------------------
...
for (current_seed in seeds) {
  ...
  cat("\n============================================================\n")
  cat("Running linear SVR for seed:", current_seed, "\n")
  cat("============================================================\n")
  ...
  for (group_name in names(stable_groups)) {
    ...
    model_result <- train_and_evaluate_svr_linear(
...
# ------------------------------------------------------------
# 13. Function for most frequent C
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 15. Learning curves for linear SVR
# ------------------------------------------------------------
# We start at 40% of the training set to avoid unstable fits
# caused by too few observations relative to the number of predictors.
...
for (current_seed in seeds) {
  ...
  cat("\n============================================================\n")
  cat("Learning curve linear SVR - seed:", current_seed, "\n")
  cat("============================================================\n")
  ...
  for (group_name in names(stable_groups)) {
    ...
    for (fraction in training_fractions) {
      ...
      lc_result <- train_and_evaluate_svr_linear(
        group_name = group_name,
        predictors = predictors,
...
p_r2 <- ggplot(
  ...
  labs(
    title = paste("Linear SVR learning curves based on R2 -", group_label),
    x = "Training fraction",
    y = "R2"
  ) +
  theme_bw()
...
p_rmse <- ggplot(
  ...
  facet_wrap(~ Stability_group, scales = "free_y") +
  labs(
    title = paste("Linear SVR learning curves based on RMSE -", group_label),
    x = "Training fraction",
    y = "RMSE"
  ) +
  theme_bw()
...
learning_curve_r2_file <- paste0(
  "SVR_linear_learning_curve_R2_",
  tolower(group_label),
  ".png"
)

learning_curve_rmse_file <- paste0(
  "SVR_linear_learning_curve_RMSE_",
  tolower(group_label),
  ".png"
)
...
output_excel_file <- paste0(
  "SVR_linear_repeated_seed_results_",
  tolower(group_label),
  ".xlsx"
)
...
cat("\n============================================================\n")
cat("Linear SVR repeated-seed modeling completed for:", group_label, "\n")
cat("============================================================\n\n")
