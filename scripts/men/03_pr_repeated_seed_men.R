# ============================================================
# Repeated-seed Polynomial Regression (PR)
# Male subgroup
# Polynomial degree selected from screening: degree 2
...
# ------------------------------------------------------------
# 9. Polynomial formula builder
# ------------------------------------------------------------
# raw = FALSE uses orthogonal polynomials.
# Degree 2 was selected during the screening phase.
...
# ------------------------------------------------------------
# 10. Train and evaluate Polynomial Regression
# ------------------------------------------------------------

train_and_evaluate_pr <- function(
    group_name,
    predictors,
    train_data,
    test_data,
  ...
  # Predictions in transformed scale
  ...
  # Back-transform predictions and observed values
...
# ------------------------------------------------------------
# 11. Run repeated-seed Polynomial Regression experiment
# ------------------------------------------------------------
...
for (current_seed in seeds) {
  ...
  cat("\n============================================================\n")
  cat("Running PR degree", polynomial_degree, "for seed:", current_seed, "\n")
  cat("============================================================\n")
  ...
  for (group_name in names(stable_groups)) {
    ...
    model_result <- train_and_evaluate_pr(
...
# ------------------------------------------------------------
# 14. Learning curves for Polynomial Regression
# ------------------------------------------------------------
# We start at 40% of the training set to avoid unstable fits
# caused by too few observations relative to the number of polynomial terms.
...
for (current_seed in seeds) {
  ...
  cat("\n============================================================\n")
  cat("Learning curve PR degree", polynomial_degree, "- seed:", current_seed, "\n")
  cat("============================================================\n")
  ...
  for (group_name in names(stable_groups)) {
    ...
    for (fraction in training_fractions) {
      ...
      lc_result <- train_and_evaluate_pr(
        group_name = group_name,
        predictors = predictors,
...
p_r2 <- ggplot(
  ...
  labs(
    title = paste("PR degree", polynomial_degree, "learning curves based on R2 -", group_label),
    x = "Training fraction",
    y = "R2"
  ) +
  theme_bw()
...
p_rmse <- ggplot(
  ...
  facet_wrap(~ Stability_group, scales = "free_y") +
  labs(
    title = paste("PR degree", polynomial_degree, "learning curves based on RMSE -", group_label),
    x = "Training fraction",
    y = "RMSE"
  ) +
  theme_bw()
...
# 16. Export to Excel
# ------------------------------------------------------------
# Model_status is kept internally for debugging purposes,
# but it is removed from the exported Excel files because
# it does not provide analytical information when all models run correctly.
...
cat("\n============================================================\n")
cat("PR degree", polynomial_degree, "repeated-seed modeling completed for:", group_label, "\n")
cat("============================================================\n\n")
