# ============================================================
# Repeated-seed Multiple Linear Regression (MLR)
# Male subgroup
...
create_mlr_formula <- function(response_variable, predictors) {
  as.formula(
...
# ------------------------------------------------------------
# 10. Train and evaluate MLR
# ------------------------------------------------------------

train_and_evaluate_mlr <- function(
    group_name,
    predictors,
    train_data,
    test_data,
  ...
  model_formula <- create_mlr_formula(
...
# ------------------------------------------------------------
# 11. Run repeated-seed MLR experiment
# ------------------------------------------------------------
...
for (current_seed in seeds) {
  ...
  cat("\n============================================================\n")
  cat("Running MLR for seed:", current_seed, "\n")
  cat("============================================================\n")
  ...
  for (group_name in names(stable_groups)) {
    ...
    model_result <- train_and_evaluate_mlr(
...
for (current_seed in seeds) {
  ...
  cat("\n============================================================\n")
  cat("Learning curve MLR - seed:", current_seed, "\n")
  cat("============================================================\n")
  ...
  for (group_name in names(stable_groups)) {
    ...
    for (fraction in training_fractions) {
      ...
      lc_result <- train_and_evaluate_mlr(
        group_name = group_name,
        predictors = predictors,
...
p_r2 <- ggplot(
  ...
  labs(
    title = paste("MLR learning curves based on R2 -", group_label),
    x = "Training fraction",
    y = "R2"
  ) +
  theme_bw()
...
p_rmse <- ggplot(
  ...
  facet_wrap(~ Stability_group, scales = "free_y") +
  labs(
    title = paste("MLR learning curves based on RMSE -", group_label),
    x = "Training fraction",
    y = "RMSE"
  ) +
  theme_bw()
...
learning_curve_r2_file <- paste0(
  "MLR_learning_curve_R2_",
  tolower(group_label),
  ".png"
)

learning_curve_rmse_file <- paste0(
  "MLR_learning_curve_RMSE_",
  tolower(group_label),
  ".png"
)
...
cat("\n============================================================\n")
cat("MLR repeated-seed modeling completed for:", group_label, "\n")
cat("============================================================\n\n")
